    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm md:text-base text-gray-500 mt-1 font-medium">Lengkapi formulir di bawah ini untuk menambahkan data siswa ke dalam sistem.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="p-6 sm:p-8 border-b border-gray-100 bg-gray-50/50">
                <div class="flex items-center gap-4 mb-2">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Form identitas pokok untuk keperluan pendaftaran.</p>
                    </div>
                </div>
            </div>

            <!-- The Form -->
            <form action="{{ route('students.store') }}" method="POST" class="p-6 sm:p-8">
                @csrf
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-6 mt-2">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="jenis_kelamin" value="L" class="w-5 h-5 text-emerald-600 focus:ring-emerald-500 border-gray-300" {{ old('jenis_kelamin', 'L') === 'L' ? 'checked' : '' }}>
                                <span class="text-sm font-medium text-gray-700">Laki-Laki (L)</span>
                            </label>
                            
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="jenis_kelamin" value="P" class="w-5 h-5 text-emerald-600 focus:ring-emerald-500 border-gray-300" {{ old('jenis_kelamin') === 'P' ? 'checked' : '' }}>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative group">
                            ...
                            <textarea name="alamat" id="alamat" rows="3"
                                class="block w-full pl-11 pr-4 py-3 bg-gray-50/50 border-gray-200 rounded-xl focus:ring-2 focus:ring-sage/20 focus:border-sage focus:bg-white transition-all shadow-sm resize-none @error('alamat') border-red-300 ring-red-100 @enderror"
                                placeholder="Masukkan alamat lengkap rumah, nama jalan, RT/RW, kelurahan...">{{ old('alamat') }}</textarea>
                        </div>
                ...
                <div class="mt-10 pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-3 bg-sage hover:bg-emerald-800 text-white font-semibold rounded-xl transition-all shadow-md shadow-sage/20 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-sage focus:ring-offset-2 flex items-center">
                        <i data-lucide="save" class="w-5 h-5 mr-2"></i> Simpan Data Siswa
                    </button>
                </div>
            </form>
        </div>
